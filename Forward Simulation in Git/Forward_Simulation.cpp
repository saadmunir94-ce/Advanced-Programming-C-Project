			...
			while (begin != pending_list.end())
			{
				...
				else
				{
					...
					case 'u':
					{
						...
						auto producer = begin->first->Release_Builder(  );
				...
				*/
				...
				if (!delay && Resources::minerals >= info.req_minerals&& Resources::gas >= info.req_gas&& Resources::supply >= info.req_supply)
				{
					...
					//	auto iter_builder = Prepared_Objects::builder_map.find(required_builder);
						if (dependency_met )
						{
						//	std::cout << "WEe are here" << std::endl;
							auto entries = Prepared_Objects::builder_map.count(required_builder);
						//	std::cout << "Value of entries is : " << entries << std::endl;
							if (!entries)
							{
							...
							else
							{
								...
								int b_delay;
								int count = 0;
								while (entries && is_busy)
								{
									if (!iter_builder->second->Building_Status())
									{
										b_delay = iter_builder->second->return_time_to_get_free();
										if (!count)
										{
